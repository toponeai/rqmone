...
CREATE POLICY "ai_agents_user_all" ON public.ai_agents FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "ai_conversations_user_all" ON public.ai_conversations FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "ai_messages_via_conversation" ON public.ai_messages FOR ALL TO authenticated
...
CREATE POLICY "ai_plans_user_all" ON public.ai_plans FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "ai_plan_tasks_via_plan" ON public.ai_plan_tasks FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.ai_plans p WHERE p.id = ai_plan_tasks.plan_id AND p.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.ai_plans p WHERE p.id = ai_plan_tasks.plan_id AND p.user_id = auth.uid()));
...
CREATE POLICY "ai_templates_select" ON public.ai_prompt_templates FOR SELECT TO authenticated USING (user_id = auth.uid() OR user_id IS NULL OR is_public = true);
CREATE POLICY "ai_templates_modify" ON public.ai_prompt_templates FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "ai_logs_user_select" ON public.ai_logs FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "ai_logs_user_insert" ON public.ai_logs FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "ai_rate_limits_user" ON public.ai_rate_limits FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE INDEX ai_rate_limits_user_window_idx ON public.ai_rate_limits (user_id, window_start);

CREATE OR REPLACE FUNCTION public.increment_ai_rate_limit(p_user_id uuid, p_window_start timestamptz, p_tokens integer DEFAULT 0)
...
$$;
GRANT EXECUTE ON FUNCTION public.increment_ai_rate_limit(uuid, timestamptz, integer) TO authenticated, service_role;

CREATE TRIGGER ai_agents_updated_at BEFORE UPDATE ON public.ai_agents FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
CREATE TRIGGER ai_plans_updated_at BEFORE UPDATE ON public.ai_plans FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
CREATE TRIGGER ai_plan_tasks_updated_at BEFORE UPDATE ON public.ai_plan_tasks FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
CREATE TRIGGER ai_prompt_templates_updated_at BEFORE UPDATE ON public.ai_prompt_templates FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
